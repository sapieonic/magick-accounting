  ...
  return (
    ...
    >
      ...
      <span className="absolute inset-0 animate-loader-orbit">
        <span
          className={clsx(
            "absolute left-1/2 top-0 -translate-x-1/2 rounded-full bg-gradient-to-br from-brand-500 to-emerald-400 shadow-[0_0_14px_rgba(47,112,69,0.4)]",
            sizeConfig.dot
          )}
        />
      </span>
      <span
        className={clsx(
          "relative rounded-full bg-gradient-to-br from-brand-600 via-brand-500 to-emerald-400 shadow-[0_0_18px_rgba(47,112,69,0.25)] animate-loader-breathe",
          sizeConfig.core
        )}
      />
    </div>
...
export function PageLoader() {
  return (
    <div className="relative flex min-h-[16rem] flex-col items-center justify-center overflow-hidden rounded-2xl border border-brand-100/70 bg-gradient-to-b from-surface via-brand-50/40 to-emerald-50/60 px-6 py-10 text-center dark:border-brand-500/20 dark:from-surface dark:via-brand-500/5 dark:to-emerald-500/5">
      ...
      <div className="relative flex flex-col items-center gap-4">
        ...
        <p className="text-xs text-muted-foreground">Serverless APIs can take a few seconds to warm up.</p>
      </div>
    </div>
  );
}
...
export function InlineLoader({
  ...
  return (
    <div className={clsx("inline-flex items-center gap-2 text-sm text-muted-foreground", className)}>
      <Spinner size="sm" />
      <span>{label}</span>
    </div>
  );
